...
#lamda function for when the time where the vacciantion will be released
#or use a critical point of when the population is in danger the vaccine will be made
#get percentage of people in each status and when the conditions are at a critical point release vaccine
    ...
    def Mututations(self):
        ...
        get the initial status of the variant of covid then update it overtime
    ...
    def get_update_count(self):
        """ returns a count of the number of people in each status
            sus, inf, recov, dead, vax                         """

             #try to show the states of different people in the grid 
             # also show which mutation is at 
             #like legend() on matplotllib

        counts = {}

    def Restrictions(self):
        """ when cases are too high and critical implement restrictions
        reduce the paramters so that the cases and infections go down 
        once the cases are at a low point release the restrictions and return 
        to initial cases 
        """
